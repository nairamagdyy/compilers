...
{input.txt file for new operation absolute difference}
read x; {input an integer}
...
enum TokenType
{
    ...
    PLUS, MINUS, ABSOLUTE_DIFFERENCE, TIMES, DIVIDE, POWER,  // add new operation (absolute difference) for TokenType
    SEMI_COLON,
...
const char* TokenTypeStr[]=
{
    ...
    "Plus", "Minus", "AbsoulteDifferenece", "Times", "Divide", "Power", // add new operation (absolute difference) used for debugging only
    "SemiColon",
...
// term -> anotherterm { & anotherterm }      left associative ------> This operator has higher precedence than +,- and lower precedence than *,/
// anotherterm -> factor { (*|/) factor } left associative
...
// anotherterm -> factor { (*|/) factor } left associative
TreeNode* anotherTerm(CompilerInfo* pci, ParseInfo* ppi)
{
    pci->debug_file.Out("Start anotherTerm");
    ...
    pci->debug_file.Out("End anotherTerm");
    return tree;
}
// term -> anotherterm { & anotherterm }      left associative
TreeNode* Term(CompilerInfo* pci, ParseInfo* ppi)
{
    pci->debug_file.Out("Start Term");

    TreeNode* tree=anotherTerm(pci, ppi);
    ...
    while(ppi->next_token.type==ABSOLUTE_DIFFERENCE)
    {
        ...
        new_tree->child[0]=tree;
        Match(pci, ppi, ppi->next_token.type);
        new_tree->child[1]=anotherTerm(pci, ppi);
...
////////////////////////////////////////////////////////////////////////////////////
// Code Generator //////////////////////////////////////////////////////////////////
// function do absolute difference
int AbsoulteDifferenece(int a,int b)
{
    if(a>=b)
        return a-b;
    else
...
int Evaluate(TreeNode* node, SymbolTable* symbol_table, int* variables)
{
    ...
    if(node->oper==MINUS)
        return a-b;
    if(node->oper==ABSOLUTE_DIFFERENCE)
        return AbsoulteDifferenece(a,b);
    if(node->oper==TIMES)
        return a*b;
    if(node->oper==DIVIDE)
        return a/b;
